...
const featureIcons = [Zap, Waves, Cpu, Bell];
const featureColors = [
  'text-sky-400 group-hover:bg-sky-500',
  'text-brand-sage group-hover:bg-brand-sage',
  'text-zinc-400 group-hover:bg-white',
  'text-red-400/70 group-hover:bg-red-500',
];
    ...
    <section id="automation" className="py-12 md:py-20 bg-[#050505] px-6 overflow-hidden relative border-b border-white/5">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-12 items-center relative z-10">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <span className="text-[9px] md:text-[10px] uppercase tracking-[0.3em] md:tracking-[0.4em] text-sky-400 font-bold mb-3 md:mb-4 block text-glow-blue">{a.eyebrow}</span>
          <h2 className="text-2xl md:text-5xl font-medium tracking-tight text-white mb-4 md:mb-6 leading-tight italic">{a.heading}</h2>
            ...
            {a.features.map((feature, index) => {
              ...
              return (
                <div key={index} className="flex gap-3 group">
                  <div className={`shrink-0 p-2.5 glass-card silver-border rounded-lg ${featureColors[index]} group-hover:text-black transition-all duration-700 h-fit`}>
                    <Icon size={18} />
                  </div>
                  <div>
                    <h4 className="text-white font-medium mb-0.5 tracking-wide uppercase text-[10px]">{feature.title}</h4>
                    <p className="text-[11px] text-zinc-500 leading-relaxed font-light">{feature.desc}</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="aspect-[4/5] md:aspect-square glass-card silver-border rounded-[1.25rem] md:rounded-[1.5rem] overflow-hidden relative">
            ...
            <div className="absolute bottom-4 md:bottom-6 left-4 md:left-6 right-4 md:right-6 p-3 md:p-4 glass-card silver-border backdrop-blur-md rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[9px] md:text-[11px] text-white uppercase tracking-[0.2em] font-medium">{a.badge}</span>
              </div>
            </div>
          </div>
        </motion.div>
